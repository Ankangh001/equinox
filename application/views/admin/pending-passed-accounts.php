...
<script>
  $('#navbar-collapse').prepend(`<h4 class="fw-bold mb-0"><span class="text-muted fw-light">User /</span> Pending Pass Accounts</h4>`);
  ...
  function loadTable(){
    ...
    $('.table').DataTable({
        ajax: "<?php echo base_url('admin/account/getPendingPassedAccounts'); ?>",
        deferRender: true,
        "pageLength": 100,
        columns:[
          {
          ...
          },
          {
            ...
            render: function (data, type, row) {
                return `<div class="d-flex justify-content-space-between">
                    <a onclick="toggleAccount('${row.id}','${row.admin_account_status}')" class="btn btn-success btn-sm" href="javascript:void(0);"><i class="bx bx-check-circle me-1"></i></a>
                  </div>`;
            }
          },
        ]
