...
class Payout extends APIMaster {
	...
	public function approvePayout(){
		try {
			$res = $this->db->where(['payout_id'=>$this->input->post('payout_id')])
			->update('payout_history', ['payout_status' => '1']);

			$get_payout = $this->db->where(['payout_id' => $this->input->post('payout_id') ])
							->get('payout_history')->result_array();
            ...
            $this->send_payout_email($email, $name, $product_category, $account_size, $account_num, $grossAmount, "APPROVED");
	...
	public function rejectPayout(){
		try {
            ...
            $this->send_payout_email($email, $name, $product_category, $account_size, $account_num, $grossAmount, "REJECT");
	...
	public function send_payout_email($user_email, $name, $product_category, $account_size, $account_num, $grossAmount, $status){
		...
		$email = send_email($user_email, 'Funded account payout status', $finaltemp,'','',3);
