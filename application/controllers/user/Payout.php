...
class Payout extends APIMaster {
	...
	public function requestPayout()
	{
		...
		if($this->input->post('payoutType') == 'Affiliate'){
			$check = $this->db->select('user.email, user.first_name, user.last_name')->where(['user_id' => $_SESSION['user_id']])->get('user')->result_array();

			$email = $check[0]['email'];
			$name = $check[0]['first_name'].' '.$check[0]['last_name'];
			$affiliateAmount = $this->input->post('payoutAmount');			
			$this->send_payout_email($email, $name, "", "", "", $affiliateAmount, "AFFILIATE");
		}else{
			...
			$this->send_payout_email($email, $name, $product_category, $account_size, $account_num, $grossAmount, "PROFITSPLIT");
		}
	...
	public function send_payout_email($user_email, $name, $product_category, $account_size, $account_num, $grossAmount, $type){
												...
												<table border="0" cellpadding="0" cellspacing="0" width="100%">
													<tr>
														<td align="left" bgcolor="#CCCCCC" width="50%" style="padding: 12px;font-family: "Source Sans Pro", Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;">
															<strong>Your payout details:</strong>
														</td>
